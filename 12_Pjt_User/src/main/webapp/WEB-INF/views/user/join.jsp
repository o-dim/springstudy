...
<script type="text/javascript">
	...
	function fnCheckId(){
		$('#id').on('keyup', function() {
			...
			// 정규식 (5~40자, 소문자 + 숫자 + 하이픈 + 밑줄 사용)
			let regId = /^[a-z0-9][a-z0-9-_]{4,39}$/;
			
			// 정규식 검사
			verifyId = regId.test(id);
			if(verifyId == false){
				$('#msgId').text('5~40자, 소문자, 숫자, 하이픈, 밑줄 사용가능, 첫 글자는 소문자, 숫자 사용가능');
				return; // 여기서 함수 실행을 종료한다 (이후에 나오는 ajax 중복체크를 위해서)
			}
	...
	function fnCheckPw() {
		$('#pw').on('keyup', function() {
			...
			// 정규식 (4~20글자, 소문자, 대문자, 숫자, 특수문자 가능)
			let pwLenth = pw.lenth;			let validCount = /[a-z]/.test(pw) // 소문자를 가지고 있으면 true(1) 없으면 false(0)
						   + /[A-Z]/.test(pw) // 대문자를 가지고 있으면 true(1) 없으면 false(0)
						   + /[0-9]/.test(pw)
						   + /[^a-zA-Z0-9]/.test(pw); // 특수문자 의미
		...
		$('#btnGetCode').on('click', function() {
			new Promise(function(resolve, reject) { 
				...
				$.ajax({
					...
					data : 'email=' + email,
					dataType : 'json',
					success : function(resData) { // resData = {"enableEmail" : true} 또 {"enableEmail" : false}
	...
	/* 	new Promise(function(resolve, reject) { 
			$.ajax({
				success: function() {
					resolve(); // 이메일 중복 체크 통과했을 
				},
				
				error : function() {
					reject(); // 이메일 중복 체크 실패했을 때
				}
			})
		}).then(function() { // resolve() 함수 호출할 때 처리되는 함수 (ajax 처리 성공했을때)
			// 인증메일 전송
			$.ajax({
				
			})
		}).catch(function(number) { // reject() 함수 호출할 때 처리되는 함수 (ajax 처리 error했을 때)
...
<body>
  ...
  <div>
    ...
    <form id="frmJoin" method="post" action="${contextPath}/user/join.do">
		...
		<div>
			<input type="text" onclick="execDaumPostcode()" id="postcode" name = "postcode" placeholder="우편번호" readonly="readonly">
